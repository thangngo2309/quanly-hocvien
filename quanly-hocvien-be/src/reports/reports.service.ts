...
export class ReportsService {
  ...
  async getSummary(filter?: {
    ...
    const expenseQb = this.expenseRepo
      ...
      .leftJoin('expense.course', 'course')
    ...
    if (courseId) {
      ...
      expenseQb.andWhere('course.id = :courseId', { courseId });
    }
  
    if (studentId) {
      enrollmentQb.andWhere('student.id = :studentId', { studentId });
      paymentQb.andWhere('student.id = :studentId', { studentId });
  
      // Khi lọc theo học viên thì chỉ lấy khoản chi gắn trực tiếp với học viên đó.
      expenseQb.andWhere('student.id = :studentId', { studentId });
    }
  ...
  async getPaymentStatus(filter?: {
    ...
    const qb = this.enrollmentRepo
      ...
      .leftJoinAndSelect('enrollment.tuition_payments', 'tuition_payments')
    ...
    return enrollments.map(enrollment => {
      ...
      return {
        ...
        first_payment_status: this.getPaymentStatusText(firstPaid, firstExpected),
  ...
  async getCourseProfit(courseId: number) {
    ...
    const expenses = await this.getExpenseDetails({ courseId });
  ...
  async getExpenseDetails(filter?: {
    courseId?: number;
    studentId?: number;
  }) {
